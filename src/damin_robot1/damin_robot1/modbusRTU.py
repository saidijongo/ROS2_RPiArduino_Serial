...
# Defining the code and value maps for different joysticks
...
class JoystickRos2(Node):
    ...
    def __init__(self):
        ...
        # Joy publisher
        self.publisher_ = self.create_publisher(Joy, 'joy', qos_profile=rclpy.qos.qos_profile_sensor_data)
    ...
    def run(self):
        ...
        while rclpy.ok():
            try:
                ...
                print('Hey Jongo, Joystick not found.Hold on, searching every second.')
                time.sleep(1)
                device_manager.find_devices()
                continue

            if gamepad.name not in JOYSTICK_CODE_VALUE_MAP:
                print('Sorry Jongo, joystick type not supported!! Please plug in supported joystick')
                time.sleep(1)
                device_manager.find_devices()
                continue
                    ...
                    for event in events:
                        if event.code in JOYSTICK_CODE_VALUE_MAP[event.device.name][0]:
                            ...
                            if event.ev_type == 'Key':
                                ...
                                self.joy.axes[key_code] = self.normalize_key_value(
                                    value_range[0], value_range[1], event.state)
